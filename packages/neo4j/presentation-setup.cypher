...
create
    ...
    (TourNavigation:Navigation {name:"Choose a Path: (Explain the App on first visit)"}),
    ...
    (Tour3:Info {name:"The cool thing is, it takes its navigation from an online Neo4j Graph Database.", image:"https://media.giphy.com/media/xTiTnxpQ3ghPiB2Hp6/giphy.gif"}),
    ...
    (Tour19:Info {name:"This approach is nestable. We can create a (:Person) that's a [:PARTICIPANT] of an (:Appointment).", image: "https://media.giphy.com/media/xTeWOVI8shjIDTq9Yk/giphy.gif"}),
    ...
    (Tour25:Info {name:"2. Ontology Graph - the Ontology of the Knowledge Graph. Provides the Structure for what the (:Factory) creates."}),
    ...
    (DataNavigation:Navigation {name:"Disclaimer: These are simplified / drafts"}),
    (PageSchema:Info {name:"This was our Draft for the Page Schema.", image: "https://raw.githubusercontent.com/omesb/skg-assets/main/img/PageSchema.png"}),
    (OntologyDraft:Info {name:"This was our Concept for the Ontology", image: "https://raw.githubusercontent.com/omesb/skg-assets/main/img/OntologyDraft.png"}),
    (PathData:Info {name:"This was some examplary Path Data, before the App was implemented", image: "https://raw.githubusercontent.com/omesb/skg-assets/main/img/PathData.png"}),
    (PathfinderData:Info {name:"This is a snapshot from our DB a few Days ago", image: "https://raw.githubusercontent.com/omesb/skg-assets/main/img/Pathfinder.png"}),

    (DeadEnd:Decision {name:"Do you want to go Back?"}),

    (Limbo:Info {name:"Path not created yet..."}),
    (Tour27) -[:NEXT {name: "Start again"}]-> (TourNavigation),
    ...
    (Start) -[:NEXT]-> (TourNavigation),
    (NormalUI1) -[:NEXT]-> (CreateSemanticDiaryDecision),
    (TourNavigation) -[:LINK {name: "Explain the App."}]-> (TourStart),
    (TourNavigation) -[:LINK {name: "Database Snapshots"}]-> (DataNavigation),
    (TourNavigation) -[:LINK {name: "Dead End"}]-> (DeadEnd),
    (TourNavigation) -[:LINK {name: "I'm bored"}]-> (Limbo),
    (TourNavigation) -[:LINK {name: "Semantic Diary."}]-> (NormalUI1),

    (DeadEnd) -[:NO]-> (WhatThen),
    (DeadEnd) -[:YES]-> (TourNavigation),

    (WhatThen) -[:NEXT {name:"MAOAM! MAOAM! MAOAM!"}]-> (Maoam),
    (Maoam) -[:NEXT]-> (TourNavigation),
    (Limbo) -[:NEXT]-> (TourNavigation),

    (DataNavigation) -[:LINK {name: "Page Schema Draft"}]-> (PageSchema),
    (DataNavigation) -[:LINK {name: "Ontology Draft"}]-> (OntologyDraft),
    (DataNavigation) -[:LINK {name: "Path Example Data"}]-> (PathData),
    (DataNavigation) -[:LINK {name: "Pathfinder Database Snapshot"}]-> (PathfinderData),
    (DataNavigation) -[:LINK {name: "Back to Navigation!"}]-> (TourNavigation),
    (PageSchema) -[:NEXT {name: "Back"}]-> (DataNavigation),
    (OntologyDraft) -[:NEXT {name: "Back"}]-> (DataNavigation),
    (PathfinderData) -[:NEXT {name: "Back"}]-> (DataNavigation),
    (PathData) -[:NEXT {name: "Back"}]-> (DataNavigation),
